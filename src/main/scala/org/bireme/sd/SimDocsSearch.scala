  ...
  private def searchIdsNoLD(text: String,
    ...
    if (maxDocs == 0) auxIds
      ...
      if (lastDays.isEmpty) {
        ...
        val ids = getIdScore(sdSearcher.search(orQuery, maxDocs * multi).scoreDocs, ngrams, analyzer, maxDocs, minNGrams)
        (auxIds ++ ids).distinct
      } else {
        val orQuery: Query = getQuery(text, sources, instances, Some(lastDays.head), useDeCS = true)
        val ids = getIdScore(sdSearcher.search(orQuery, maxDocs * multi).scoreDocs, ngrams, analyzer, maxDocs, minNGrams)
        val nids = (auxIds ++ ids).distinct
        searchIdsNoLD(text, sources, instances, maxDocs - nids.size, ngrams, minNGrams, analyzer, lastDays.tail, nids)
      }
    }
  }
